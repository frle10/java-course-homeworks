 ...
 */
public class SymbolShellCommand extends Command implements ShellCommand {
	...
	public SymbolShellCommand() {
		super("symbol");
		getDescription().add("If provided with 1 argument, prints the symbol for given symbol type.");
		getDescription().add("If provided with 2 arguments, changes the symbol for given symbol type.\n");
		getDescription().add("Possible symbol types: PROMPT, MORELINES, MULTILINE");
	}
	...
	public ShellStatus executeCommand(Environment env, String arguments) {
		...
		if(args.length == 2) {
			...
			// check symbol type validity and if new symbol is a character or not
			...
			// actually set the new symbol for given symbol type
