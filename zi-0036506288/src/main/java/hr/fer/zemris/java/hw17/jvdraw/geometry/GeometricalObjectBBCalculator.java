...
public class GeometricalObjectBBCalculator implements GeometricalObjectVisitor {
	...
	public void visit(FilledTriangle filledTriangle) {
		...
		int minX = min(min(firstPoint.getX(), secondPoint.getX()), thirdPoint.getX());
		int minY = min(min(firstPoint.getY(), secondPoint.getY()), thirdPoint.getY());
		int xMax = max(max(firstPoint.getX(), secondPoint.getX()), thirdPoint.getX());
		int yMax = max(max(firstPoint.getY(), secondPoint.getY()), thirdPoint.getY());
		
		int width = xMax - minX;
		int height = yMax - minY;
		
		Rectangle filledTriangleBounds = new Rectangle(minX, minY, width, height);
		updateBoundingBox(filledTriangleBounds);
	}
